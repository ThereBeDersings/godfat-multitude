...
- Currently, the tool does not track what the last cat rolled was. If 1A is a Rare dupe (from 0A), it won't be able to discern that, affects both rolling mode and planning mode. Not an issue if you know what to do (backtrack if planning, or just clicking on the Rare dupe and skip the first Rare if rolling).
- The very obvious issue of cross-banner track switches: they're simply blocked. A bit of a hassle to work around, especially in planning mode.
