...
- Currently, the tool has does not track what the last cat rolled was. If 1A is a Rare dupe (from 0A), it won't be able to discern that, affects both rolling mode and planning mode (working on it on the test repo).
- The very obvious issue of cross-banner track switches. They're simply blocked (also working on it on the test repo).
